  ...
  /* USER CODE BEGIN WHILE */
    while (1) {
        ...
        if (state_tim_counter % STATE_TIM_10 == 0) {
            ...
            // echo data log params
            if (receive_frame.cmd == DATALOG_SET_LOG_DATA_CMD) {
                uint8_array_to_name_string(receive_frame.data,
                                           param_name_string_array,
                                           receive_frame.len - PROTOCOL_FRAME_HEADER_SIZE - PROTOCOL_FRAME_CHECKSUM_SIZE);
                send_frame.cmd = DATALOG_ECHO_LOG_DATA_CMD;
                name_string_to_uint8_array(param_name_string_array,
                                           param_data_array,
                                           strlen(param_name_string_array));
                send_frame.len = PROTOCOL_FRAME_HEADER_SIZE + PROTOCOL_FRAME_CHECKSUM_SIZE + strlen(param_name_string_array);
